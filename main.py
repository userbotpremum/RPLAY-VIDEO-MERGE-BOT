...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "mergeNow" in cb.data:
        ...
        await cb.message.edit(
            text=Config.START_TEXT,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Developer - @BullMovies_Team", url="https://t.me/BullMovies_Team")], [InlineKeyboardButton("CONTACT BOT", url="https://t.me/BullMoviesOfficial_bot")], [InlineKeyboardButton("MOVIE CHANNEL", url="https://t.me/BullMovieTeam"),InlineKeyboardButton("MOVIE STICKERS", url="https://t.me/addstickers/IndianBot_1167961858_kang")], [InlineKeyboardButton("SUPPORT GROUP", url="https://t.me/BullMoviesTorrent")]]),
            disable_web_page_preview=True
        )
        ...
        if (QueueDB.get(cb.from_user.id, None) is None) or (QueueDB.get(cb.from_user.id) == []):
            await cb.answer("Sorry Unkil, Your Queue is Empty!", show_alert=True)
            return
        merged_vid_path = f"{Config.DOWN_PATH}/{str(cb.from_user.id)}/COBRA MOVIES TEAM.{FormtDB.get(cb.from_user.id).lower()}"
